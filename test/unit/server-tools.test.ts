...
const mockHandleWhatsNext = vi.fn().mockImplementation(async (args) => {
  ...
  mockGetConversationContext.mockRejectedValueOnce(new Error('No development conversation exists'));
...
describe('Server Tools', () => {
  ...
  describe('whats_next tool', () => {
    it('should return helpful error when no conversation exists', async () => {
      ...
      expect(result.message).toContain('No development conversation has been started');
      expect(result.instructions).toContain('Please use the start_development tool');
  ...
  describe('proceed_to_phase tool', () => {
    it('should return helpful error when no conversation exists', async () => {
      mockGetConversationContext.mockRejectedValueOnce(
        new Error('No development conversation exists for this project. Use the start_development tool first to initialize development with a workflow.')
      );
      ...
      // Verify result contains helpful error message
      expect(result).toHaveProperty('error', true);
      expect(result).toHaveProperty('message', 'No development conversation has been started for this project.');
      expect(result).toHaveProperty('instructions', 'Please use the start_development tool first to initialize development with a workflow.');
  ...
  describe('start_development tool', () => {
    it('should create a new conversation with specified workflow', async () => {
      ...
      // Call the tool handler
      const result = await server.handleStartDevelopment({ workflow: 'waterfall' });
