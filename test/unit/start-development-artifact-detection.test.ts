...
import { MockContextFactory, TEST_WORKFLOWS, TestAssertions } from '../utils/test-helpers.js';
...
describe('StartDevelopmentHandler - Dynamic Artifact Detection', () => {
  ...
  beforeEach(() => {
    ...
    mockProjectDocsManager = MockContextFactory.createProjectDocsManagerMock(testProjectPath);
  ...
  describe('dynamic workflow analysis', () => {
    it('should proceed normally when workflow contains no document variables', async () => {
      mockContext.workflowManager.loadWorkflowForProject.mockReturnValue(TEST_WORKFLOWS.simple);

      const result = await handler.executeHandler({ workflow: 'simple-workflow' }, mockContext);
    ...
    it('should detect and validate only referenced document variables', async () => {
      mockContext.workflowManager.loadWorkflowForProject.mockReturnValue(TEST_WORKFLOWS.withArchDoc);
      
      // Update mock context to match the workflow's initial state
      mockContext.conversationManager.createConversationContext = vi.fn().mockResolvedValue({
      ...
      mockProjectDocsManager.getProjectDocsInfo.mockResolvedValue({
        architecture: { exists: false, path: join(testProjectPath, '.vibe', 'docs', 'architecture.md') },
        requirements: { exists: true, path: join(testProjectPath, '.vibe', 'docs', 'requirements.md') },
        design: { exists: true, path: join(testProjectPath, '.vibe', 'docs', 'design.md') }
      });

      const result = await handler.executeHandler({ workflow: 'arch-focused' }, mockContext);
      
      TestAssertions.expectArtifactSetupPhase(result);
      expect(result.instructions).toContain('**Referenced Variables:** `$ARCHITECTURE_DOC`');
    ...
    it('should detect multiple document variables in workflow', async () => {
      mockContext.workflowManager.loadWorkflowForProject.mockReturnValue(TEST_WORKFLOWS.withMultipleDocs);
      ...
      mockProjectDocsManager.getProjectDocsInfo.mockResolvedValue({
        architecture: { exists: false, path: join(testProjectPath, '.vibe', 'docs', 'architecture.md') },
        requirements: { exists: false, path: join(testProjectPath, '.vibe', 'docs', 'requirements.md') },
        design: { exists: false, path: join(testProjectPath, '.vibe', 'docs', 'design.md') }
      });

      const result = await handler.executeHandler({ workflow: 'multi-doc' }, mockContext);
      
      TestAssertions.expectArtifactSetupPhase(result);
      expect(result.instructions).toContain('**Referenced Variables:** `$ARCHITECTURE_DOC`, `$REQUIREMENTS_DOC`, `$DESIGN_DOC`');
      expect(result.instructions).toContain('architecture.md');
      expect(result.instructions).toContain('requirements.md');
      expect(result.instructions).toContain('design.md');
    });
    ...
    it('should proceed normally when all referenced documents exist', async () => {
      ...
      mockProjectDocsManager.getProjectDocsInfo.mockResolvedValue({
        architecture: { exists: true, path: join(testProjectPath, '.vibe', 'docs', 'architecture.md') },
        requirements: { exists: true, path: join(testProjectPath, '.vibe', 'docs', 'requirements.md') },
        design: { exists: true, path: join(testProjectPath, '.vibe', 'docs', 'design.md') }
      });

      const result = await handler.executeHandler({ workflow: 'complete-docs' }, mockContext);
      
      expect(mockProjectDocsManager.getProjectDocsInfo).toHaveBeenCalledWith(testProjectPath);
    ...
    it('should handle partial document availability correctly', async () => {
      ...
      const partialWorkflow = {
        ...
        states: {
          development: {
            instructions: 'Implement features based on $REQUIREMENTS_DOC and follow $DESIGN_DOC patterns.'
          }
        }
      };
      
      mockContext.workflowManager.loadWorkflowForProject.mockReturnValue(partialWorkflow);
      ...
      const result = await handler.executeHandler({ workflow: 'req-design-focused' }, mockContext);
      
      TestAssertions.expectArtifactSetupPhase(result);
      expect(result.instructions).toContain('**Referenced Variables:** `$REQUIREMENTS_DOC`, `$DESIGN_DOC`');
    ...
    it('should handle workflow loading errors gracefully', async () => {
      ...
      const result = await handler.executeHandler({ workflow: 'invalid-workflow' }, mockContext);
    ...
    it('should include detected variables in setup guidance', async () => {
      ...
      const result = await handler.executeHandler({ workflow: 'arch-only' }, mockContext);
      
      expect(result.instructions).toContain('**Referenced Variables:** `$ARCHITECTURE_DOC`');
      expect(result.instructions).toContain('detected variables: `$ARCHITECTURE_DOC`');
    });
  });
});
