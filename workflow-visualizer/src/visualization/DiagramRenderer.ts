...
import { DiagramNode, DiagramLink, DiagramConfig, DiagramStyle, InteractionEvent } from '../types/visualization-types';
...
export class DiagramRenderer {
   ...
   */
  private createLinks(workflow: YamlStateMachine, _nodes: DiagramNode[]): DiagramLink[] {
  ...
  private renderDiagram(): void {
    ...
    this.transitionRenderer.render(this.g, this.links, this.handleElementInteraction.bind(this));
    
    // Render states
    this.stateRenderer.render(this.g, this.nodes, this.handleElementInteraction.bind(this));
  }
  ...
  public fitToView(): void {
    ...
    const scale = Math.min(
      width / (bounds.width + this.config.padding.left + this.config.padding.right),
      height / (bounds.height + this.config.padding.top + this.config.padding.bottom)
    ) * 0.9; // Add some margin
