...
export class TemplateManager {
  ...
  async getDefaults(): Promise<Required<TemplateOptions>> {
    ...
    return {
      architecture: this.getPreferredTemplate(availableTemplates.architecture, ['arc42', 'freestyle']),
      requirements: this.getPreferredTemplate(availableTemplates.requirements, ['ears', 'freestyle']),
      design: this.getPreferredTemplate(availableTemplates.design, ['comprehensive', 'freestyle'])
    };
  }
   ...
   */
  private getPreferredTemplate(available: string[], preferences: string[]): string {
   ...
   */
  async loadTemplate(type: 'architecture' | 'requirements' | 'design', template: string): Promise<TemplateResult> {
    ...
    try {
      ...
      logger.error(`Failed to load template: ${type}/${template}`, error as Error);
   ...
   */
  private async loadDirectoryTemplate(templatePath: string): Promise<TemplateResult> {
  ...
  async validateOptions(options: TemplateOptions): Promise<void> {
    ...
    if (options.requirements && !availableTemplates.requirements.includes(options.requirements)) {
