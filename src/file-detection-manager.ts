...
export class FileDetectionManager {
  ...
  async detectDocumentationFiles(): Promise<FileDetectionResult> {
    logger.debug('Starting documentation file detection', { projectPath: this.projectPath });
   ...
   */
  private async scanLocation(location: string): Promise<Array<{path: string, relativePath: string}>> {
    try {
  ...
  private matchFileToPatterns(
    ...
    patterns: ReturnType<typeof PathValidationUtils.getCommonDocumentationPatterns>
  ): Array<{type: 'architecture' | 'requirements' | 'design', confidence: 'high' | 'medium' | 'low'}> {
    const fileName = basename(file.path).toLowerCase();
    const relativePath = file.relativePath.toLowerCase();
    const matches: Array<{type: 'architecture' | 'requirements' | 'design', confidence: 'high' | 'medium' | 'low'}> = [];
   ...
   */
  private matchesPatterns(fileName: string, relativePath: string, patterns: string[]): boolean {
   ...
   */
  private getConfidence(fileName: string, type: string): 'high' | 'medium' | 'low' {
  ...
  private sortAndDeduplicate(files: DetectedFile[]): DetectedFile[] {
    ...
    const unique = files.filter((file, index, array) => 
